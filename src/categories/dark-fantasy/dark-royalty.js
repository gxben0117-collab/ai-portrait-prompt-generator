/**
 * 暗黑王族｜哥德宮廷｜吸血族裔
 * Dark Royalty - Gothic Court - Vampire Lineage
 */

export const DARK_ROYALTY_CATEGORY = {
  id: 'dark-royalty',
  name: '暗黑王族｜哥德宮廷｜吸血族裔',
  themeGroup: 'dark-fantasy',

  visualDNA: {
    atmosphere: [
      'dark gothic royal court',
      'vampire aristocracy elegance',
      'midnight palace grandeur',
      'immortal noble presence',
      'ancient vampire dynasty',
      'bloodline royalty',
      'centuries-old sovereign',
      'eternal night empire',
      'immortal aristocracy',
    ],

    colors: {
      primary: ['deep crimson', 'midnight black', 'royal purple'],
      secondary: ['blood red', 'silver moonlight', 'gothic gold'],
      accent: ['ruby gemstone', 'vampire eye glow'],
    },

    iconicProps: [
      'gothic throne',
      'blood-red wine goblet',
      'ornate mirror',
      'black rose arrangements',
      'crimson velvet drapes',
      'silver candelabra',
      'gothic crown and scepter',
    ],

    costumeStyle: {
      layer1: 'aristocratic foundation with vampire elegance',
      concept: 'gothic vampire royalty ceremonial dress',
      keywords: [
        'high-collar gothic royal gown',
        'crimson and black color scheme',
        'ornate vampire noble accessories',
        'dramatic cape or cloak',
        'gothic crown or tiara',
        'aristocratic jewelry',
      ],
      fabric: [
        'luxurious velvet',
        'flowing black silk',
        'crimson satin',
      ],
    },

    bodyRequirements: {
      bodyType: 'aristocratic vampire elegance',
      emphasis: 'immortal noble bearing',
    },

    sceneTypes: [
      'gothic throne room',
      'midnight ballroom',
      'vampire castle interior',
      'moonlit palace courtyard',
    ],

    lighting: {
      keyLight: 'cold moonlight through gothic windows',
      fillLight: 'candlelight warm contrast',
      rimLight: 'silver edge separation on dark costume',
      ambience: 'gothic vampire court atmosphere',
      specialEffect: 'subtle vampire eye glow',
    },

    photography: {
      mood: 'dramatic royal portrait',
      focus: 'gothic vampire cinema aesthetic',
      mood: 'dark fantasy aristocratic epic',
    },

    prohibitions: [
      'no anime vampire costume',
      'no horror monster appearance',
      'must maintain aristocratic elegance',
      'no cheap fantasy costume',
      'no low-budget cosplay',
      'no plastic armor',
      'no theme-park costume',
    ],

    textureRequirements: [
      'luxurious velvet fabric depth',
      'gothic metal ornament quality',
      'candlelight atmospheric glow',
    ],
  },

  promptWeights: {
    faceLock: 1.5,
    scene: 1.2,
    costume: 1.3,
    lighting: 1.2,
    style: 1.0,
  },
};
