...
import { generateUserControlPrompt, getUserControlDescription } from './userControlEngine.js';
...
export function buildPrompt(category, roleCard, options = {}) {
  ...
  // === Layer 0: User Controls (用戶自訂參數) ===
  const userControlPrompt = generateUserControlPrompt(userControls);
  const userControlDesc = getUserControlDescription(userControls);
  ...
  if (!sceneValidation.valid) {
    console.warn('⚠️ Scene validation failed:', sceneValidation.errors);
  }
  ...
  if (!costumeValidation.valid) {
    console.warn('⚠️ Costume validation failed:', costumeValidation.errors);
  }
  ...
  if (conflictCheck.hasConflict) {
    console.warn('⚠️ Style conflicts detected:', conflictCheck.conflicts);
  }
  ...
  return {
    ...
    metadata: {
      ...
      userControls: userControlDesc,
      aspectRatio: userControlPrompt.metadata?.aspectRatio,
      layers: {
        faceLock: faceLock.positive.substring(0, 50) + '...',
        userControls: userControlPrompt.positive.substring(0, 50) + '...',
